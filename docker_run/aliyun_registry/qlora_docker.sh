...
docker run -dt --name easy_qlora --shm-size 64g --restart=always --gpus all --network=host \
-v "$CODE_DIR":/workspace/code \
-v "$INPUT_DIR":/workspace/input \
-v "$OUTPUT_DIR":/workspace/output \
-v "${current_dir}/finetune/qlora":/workspace/easy_llm_finetuner \
-w /workspace \
registry.cn-hangzhou.aliyuncs.com/llm_dockers/qlora:0.0.1 \
/bin/bash
